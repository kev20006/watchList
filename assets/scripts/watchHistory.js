const watchHistory = {
    render(){
        closeDrawerMenu()
        $("#watch-list").html("");
        $("#watch-list").append(watchHistory.getCurrentListStatsHTML(), watchHistory.historyCard("movie"), watchHistory.historyCard("tv"));
    },
            ...
            let genreHeader = $(`
                <div class="col-12 mx-0">
                    <div class="col-12 genre d-flex justify-content-between my-1">
                        ...
                        data-target="#${element.replace(/[^A-Za-z0-9]+/g, '-')}-items" aria-expanded="false" aria-controls="${element.replace(/[^A-Za-z0-9]+/g, '-')}-items">
    ...
    historyCard: (type) =>{
        let history = $(`
            <article class="card info-card p-3">
                ...
                <section>
                    <p>You have watched <strong>${watchList.analytics[type].likes + watchList.analytics.movie.dislikes}</strong> movies on Watch List</p>
